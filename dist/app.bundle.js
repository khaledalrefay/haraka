(() => {
  // js/presentation/fit-titles.mjs
  function observeTitles(root = document.body) {
    let frame;
    const fit = (el) => {
      if (!el.clientWidth) return;
      el.classList.remove("title-expanded");
      el.style.removeProperty("font-size");
      const initial = parseFloat(getComputedStyle(el).fontSize), floor = initial * 0.82;
      let size = initial;
    ...
    const scan = () => {
      ...
      frame = requestAnimationFrame(() => root.querySelectorAll("[data-fit-title]").forEach(fit));
    };
  ...
  function renderSessionPreview(snapshot, { embedded = false } = {}) {
    ...
    return `<section class="v-session-preview">${embedded ? "" : pageHeading("\u0645\u0639\u0627\u064A\u0646\u0629 \u0627\u0644\u062C\u0644\u0633\u0629", button("\u0631\u062C\u0648\u0639", "preview-back", "text-btn"))}<div class="v-preview-intro"><span class="eyebrow">${names[w.program]} \xB7 ${levelName(w.level)}</span><h2>${esc(sessionName(w.program, w.session))}</h2><div class="v-preview-stats"><span>${estimates[w.id].join("\u2013")} \u062F\u0642\u064A\u0642\u0629 \u062A\u0642\u062F\u064A\u0631\u064A\u064B\u0627</span><span>${new Set(work.map((s) => s.exerciseId)).size} \u062A\u0645\u0627\u0631\u064A\u0646 \u0645\u062E\u062A\u0644\u0641\u0629</span><span>${type}</span></div><p class="small muted">\u0627\u0644\u0645\u062F\u0629 \u062A\u0634\u0645\u0644 \u0627\u0644\u0627\u0633\u062A\u0631\u0627\u062D\u0627\u062A \u0648\u062A\u062A\u063A\u064A\u0631 \u062D\u0633\u0628 \u0633\u0631\u0639\u0629 \u0627\u0644\u0623\u062F\u0627\u0621 \u0648\u062A\u0645\u062F\u064A\u062F \u0627\u0644\u0648\u0642\u062A.</p></div><div class="v-flow">${blocks.map((block, index) => {
      ...
      return `<section class="v-flow-phase phase-${block.id}"><header class="v-flow-phase-head"><span class="v-phase-number">${String(index + 1).padStart(2, "0")}</span><div><h2>${title}</h2><p>${description}</p></div></header><ol class="v-flow-steps">${block.steps.map((s) => {
        ...
        return `${divider}<li class="v-flow-work" data-step-id="${esc(s.id)}"><span class="v-flow-node" aria-hidden="true">${++number}</span><article><div class="v-flow-meta">${s.round ? `\u0627\u0644\u0645\u062D\u0637\u0629 ${s.station} \u0645\u0646 ${s.stations}` : s.set ? `\u0627\u0644\u0645\u062C\u0645\u0648\u0639\u0629 ${s.set} \u0645\u0646 ${s.sets}` : "\u062A\u0645\u0631\u064A\u0646"}</div><h3 data-fit-title>${esc(movementName(s, snapshot.exercises))}</h3><span class="v-flow-goal">${goal(s)}</span>${notes.length ? `<p class="v-flow-notes">${notes.map(esc).join(" \xB7 ")}</p>` : ""}</article></li>`;
      }).join("")}</ol></section>`;
    }).join("")}<div class="v-flow-end"><span aria-hidden="true">\u2713</span><strong>\u0646\u0647\u0627\u064A\u0629 \u0627\u0644\u062C\u0644\u0633\u0629</strong></div></div></section>`;
  }

  // js/presentation/program-details.mjs
  function renderProgramDetails(snapshot, heading) {
    const { program, level, session } = snapshot.workout;
    const tab = (value, label, kind, selected2) => `<button id="${kind}-tab-${value}" role="tab" aria-selected="${selected2}" aria-controls="${kind}-panel" tabindex="${selected2 ? 0 : -1}" data-action="detail-${kind}" data-id="${program}" data-level-id="${level}" data-session="${session}" data-value="${value}">${label}</button>`;
  ...
  function opportunity(settings2, history2, day = todayKey()) {
    ...
    const slot = slotFor(settings2, day) || (weekday(day) !== 0 ? slotFor(settings2, shift(day, -1)) : null);
  ...
  var weekStart = (day) => shift(day, -weekday(day));
  var samePlan = (a, b) => Boolean(a && b && a.program === b.program && a.level === b.level && a.schedule.length === b.schedule.length && a.schedule.every((x) => b.schedule.some((y) => x.day === y.day && x.session === y.session)));
  function schedulePlan(settings2, plan, day, timing) {
    if (!validPlan(plan)) throw Error("\u0627\u062E\u062A\u0631 \u062B\u0644\u0627\u062B\u0629 \u0623\u064A\u0627\u0645 \u0645\u062E\u062A\u0644\u0641\u0629 \u0648\u062C\u0644\u0633\u0627\u062A A \u0648B \u0648C \u062F\u0648\u0646 \u062A\u0643\u0631\u0627\u0631.");
    if (!["this-week", "next-week"].includes(timing)) throw Error("\u0627\u062E\u062A\u0631 \u0645\u0648\u0639\u062F \u062A\u0637\u0628\u064A\u0642 \u0627\u0644\u062E\u0637\u0629.");
    if (samePlan(settings2.revisions.at(-1), plan)) return { settings: settings2, resetRange: null };
    const start = weekStart(day), effectiveFrom = timing === "this-week" ? start : shift(start, 7);
    return { settings: { ...settings2, levels: { ...settings2.levels, [plan.program]: plan.level }, revisions: [...settings2.revisions.filter((r) => r.effectiveFrom < effectiveFrom), { ...plan, effectiveFrom }] }, resetRange: timing === "this-week" ? { start, end: shift(start, 6) } : null };
  }
  ...
  function renderHome({ settings: settings2, selected: selected2, active: active2, history: history2 }) {
    ...
    const makeup = offer?.kind === "makeup" && !active2;
    const pending = settings2.revisions.find((r) => r.effectiveFrom > t);
    let card;
    ...
    }).join("")}</div></section>${makeup ? `<section class="v-makeup-card"><span class="pill">\u062C\u0644\u0633\u0629 \u0641\u0627\u0626\u062A\u0629 \u0645\u062A\u0627\u062D\u0629 \u0627\u0644\u064A\u0648\u0645</span><h2>${sessionName(offer.program, offer.session)}</h2><p>\u0645\u0648\u0639\u062F\u0647\u0627 \u0627\u0644\u0623\u0635\u0644\u064A: ${days[weekday(offer.date)]} \xB7 ${offer.date}</p>${button("\u0627\u0628\u062F\u0623 \u062A\u0639\u0648\u064A\u0636 \u0627\u0644\u062C\u0644\u0633\u0629", "start", "primary full", `data-workout="${offer.workoutId}"`)}</section>` : ""}<section class="session-hero v-hero">${card}</section>${pending ? `<p class="v-pending-plan">\u062E\u0637\u0629 \u062C\u062F\u064A\u062F\u0629 \u062A\u0628\u062F\u0623 \u064A\u0648\u0645 \u0627\u0644\u0623\u062D\u062F ${pending.effectiveFrom}</p>` : ""}</div>`;
  }
  ...
  function renderSettings({ settings: settings2, formDraft: formDraft2, settingsPage: settingsPage2, draftProgram: draftProgram2, active: active2, history: history2, programCards: programCards2, levels: levels2, requirement: requirement2, reminderSection: reminderSection2 }) {
    ...
    if (settingsPage2 === "plan") return `<section class="v-settings">${pageHeading("\u0628\u0631\u0646\u0627\u0645\u062C\u064A", button("\u0631\u062C\u0648\u0639", "settings-back", "text-btn"))}<form id="settings-form" data-section="plan"><h2 class="v-space">\u0627\u0644\u0628\u0631\u0646\u0627\u0645\u062C \u0648\u0627\u0644\u0645\u0633\u062A\u0648\u0649</h2><div class="v-choices">${programCards2()}</div><div class="v-choices v-levels">${levels2()}</div><p>${requirement2()}</p><p class="small muted">\u0628\u0639\u062F \u0627\u0644\u062D\u0641\u0638\u060C \u0627\u062E\u062A\u0631 \u0625\u0639\u0627\u062F\u0629 \u0636\u0628\u0637 \u0647\u0630\u0627 \u0627\u0644\u0623\u0633\u0628\u0648\u0639 \u0648\u062A\u0637\u0628\u064A\u0642 \u0627\u0644\u062E\u0637\u0629\u060C \u0623\u0648 \u0628\u062F\u0621 \u0627\u0644\u062E\u0637\u0629 \u0627\u0644\u062C\u062F\u064A\u062F\u0629 \u064A\u0648\u0645 \u0627\u0644\u0623\u062D\u062F \u0627\u0644\u0642\u0627\u062F\u0645 \u0645\u0639 \u0627\u0644\u0627\u062D\u062A\u0641\u0627\u0638 \u0628\u0628\u064A\u0627\u0646\u0627\u062A \u0647\u0630\u0627 \u0627\u0644\u0623\u0633\u0628\u0648\u0639.</p>${button("\u0645\u0639\u0627\u064A\u0646\u0629 \u0627\u0644\u062C\u0644\u0633\u0627\u062A", "preview-plan", "text-btn", 'type="button"')}<h2 class="v-space">\u062C\u062F\u0648\u0644\u064A \u0627\u0644\u0623\u0633\u0628\u0648\u0639\u064A</h2><p class="small muted">\u0627\u062E\u062A\u064A\u0627\u0631 \u064A\u0648\u0645 \u0645\u0634\u063A\u0648\u0644 \u064A\u0628\u062F\u0651\u0644 \u064A\u0648\u0645\u064A \u0627\u0644\u062C\u0644\u0633\u062A\u064A\u0646 \u062A\u0644\u0642\u0627\u0626\u064A\u064B\u0627.</p><div class="v-schedule">${["A", "B", "C"].map((letter) => `<label><span>${sessionName(draftProgram2, letter)}</span><select name="day-${letter}">${days.map((name, i) => `<option value="${i}" ${Number(draft[`day-${letter}`] ?? plan.schedule.find((s) => s.session === letter)?.day) === i ? "selected" : ""}>${name}</option>`).join("")}</select></label>`).join("")}</div><button class="primary full v-space" type="submit">\u0627\u0639\u062A\u0645\u0627\u062F \u0627\u0644\u0628\u0631\u0646\u0627\u0645\u062C \u0648\u0627\u0644\u062C\u062F\u0648\u0644</button></form></section>`;
    return `<section class="v-settings">${pageHeading("\u0627\u0644\u0625\u0639\u062F\u0627\u062F\u0627\u062A")}${button(`<span><strong>\u0628\u0631\u0646\u0627\u0645\u062C\u064A</strong><small>${names[plan.program]} \xB7 ${levelName(plan.level)}</small></span><span aria-hidden="true">\u2190</span>`, "settings-plan", "v-settings-link")}<form id="settings-form" class="v-appearance-card" data-section="appearance"><h2 class="v-space">\u0627\u0644\u0645\u0638\u0647\u0631 \u0648\u0627\u0644\u0635\u0648\u062A</h2>${palettePicker(draft.palette ?? settings2.palette)}<label>\u0646\u063A\u0645\u0629 \u0627\u0644\u0645\u0624\u0642\u062A<div class="v-sound-picker"><select name="sound">${sounds.map(([id, name]) => `<option value="${id}" ${(draft.sound ?? settings2.sound) === id ? "selected" : ""}>${name}</option>`).join("")}</select>${button(icon("play"), "test-sound", "icon-btn", 'type="button" aria-label="\u0627\u0633\u062A\u0645\u0639 \u0644\u0644\u0646\u063A\u0645\u0629" title="\u0627\u0633\u062A\u0645\u0639 \u0644\u0644\u0646\u063A\u0645\u0629"')}</div></label><div class="v-buttons v-space"><button class="primary" type="submit">\u062D\u0641\u0638 \u0627\u0644\u0645\u0638\u0647\u0631 \u0648\u0627\u0644\u0635\u0648\u062A</button>${button("\u0625\u0644\u063A\u0627\u0621 \u0627\u0644\u062A\u063A\u064A\u064A\u0631\u0627\u062A", "appearance-cancel", "secondary", 'type="button"')}</div></form><hr>${reminderSection2()}<hr><section class="v-space"><h2>\u0646\u0633\u062E\u0629 \u0627\u062D\u062A\u064A\u0627\u0637\u064A\u0629</h2><p class="muted">\u0627\u062D\u0641\u0638 \u0625\u0639\u062F\u0627\u062F\u0627\u062A\u0643 \u0648\u0633\u062C\u0644\u0651\u0643 \u0648\u0627\u0644\u062C\u0644\u0633\u0629 \u0627\u0644\u062C\u0627\u0631\u064A\u0629 \u0641\u064A \u0645\u0644\u0641.</p><div class="v-backup-actions">${button("\u062A\u0635\u062F\u064A\u0631 \u0627\u0644\u0628\u064A\u0627\u0646\u0627\u062A", "export")}${button("\u0627\u0633\u062A\u0639\u0627\u062F\u0629 \u0645\u0646 \u0645\u0644\u0641", "import")}</div><input id="backup-file" type="file" accept="application/json,.json" hidden></section><hr><h2 class="v-space">\u0627\u0644\u062A\u0637\u0628\u064A\u0642</h2>${button("\u062A\u062B\u0628\u064A\u062A \u062D\u0631\u0643\u0629", "install-app")}<p id="install-status" class="small muted"></p><p class="small muted v-space">\u0627\u0644\u0625\u0635\u062F\u0627\u0631 ${APP_VERSION} \xB7 \u0627\u0644\u0635\u0648\u0631 \u0627\u0644\u062A\u0648\u0636\u064A\u062D\u064A\u0629 \u0642\u064A\u062F \u0627\u0644\u062A\u062C\u0647\u064A\u0632</p></section>`;
  }
  ...
  function libraryCard(e) {
    return `<button class="exercise-card" data-action="instruction" data-id="${e.id}"><span class="v-library-icon" aria-hidden="true">${icon("motion")}</span><span class="exercise-caption"><strong dir="auto" data-fit-title>${esc(libraryName(e))}</strong>${e.id === "march" ? '<span class="v-library-note">\u0647\u0627\u062F\u0626 \xB7 \u0645\u062A\u062F\u0631\u062C \xB7 \u0646\u0634\u064A\u0637 \xB7 \u0628\u0637\u064A\u0621</span>' : ""}<small>${e.equipment === "\u0644\u0627 \u0634\u064A\u0621" ? "\u062F\u0648\u0646 \u0623\u062F\u0648\u0627\u062A" : esc(e.equipment)}</small></span><span class="v-library-arrow" aria-hidden="true"><svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2"><path d="m14 6-6 6 6 6"/></svg></span></button>`;
  }
  ...
  function preferences(device = readDevice()) {
    ...
    return {
      ...
      doneKeys: [...new Set([...history2].sort((a, b) => (a.scheduledDate || a.dateKey).localeCompare(b.scheduledDate || b.dateKey)).map((r) => occurrence(r.scheduledDate || r.dateKey, r.snapshot.workout.session)))].slice(-100),
  ...
  var Repository = class {
    ...
    savePlan(settings2, { resetDay = null, resetRange = null } = {}) {
      return this.transaction(["settings", "active", "history"], "readwrite", async (s) => {
        ...
        if (resetRange) {
          const inside = (date2) => date2 && date2 >= resetRange.start && date2 <= resetRange.end;
          const active2 = await request(s("active").get("current"));
          if (active2 && [active2.dateKey, active2.scheduledDate].some(inside)) await request(s("active").delete("current"));
  ...
  var WorkoutService = class {
    ...
    savePlan(settings2, plan, day = todayKey(), timing = null) {
      const op = this.queue.then(async () => {
        if (timing) {
          const current = await this.repository.getSettings() || settings2;
          const change = schedulePlan(current, plan, day, timing);
          if (change.settings !== current) await this.repository.savePlan(change.settings, { resetRange: change.resetRange });
  ...
  function show(html) {
    ...
    $3("#modal").classList.toggle("wide", html.includes("v-program-details"));
  ...
  function refreshClock() {
    ...
    if (s.type !== "rest") {
      const btn = $3("#timer-button");
      btn?.style.setProperty("--fill", `${ratio * 100}%`);
      btn?.classList.toggle("timer-running", Boolean(active.timer?.running && ms > 0));
      btn?.classList.toggle("timer-complete", ms <= 0);
      btn?.classList.toggle("timer-has-progress", ratio > 0);
    }
  ...
  document.addEventListener("submit", (e) => {
    ...
    run(async () => {
      ...
      await refreshState();
      if (samePlan(settings.revisions.at(-1), plan)) {
        toast2("\u0644\u0627 \u062A\u0648\u062C\u062F \u062A\u063A\u064A\u064A\u0631\u0627\u062A \u0641\u064A \u0627\u0644\u062E\u0637\u0629");
        return;
      }
      pendingPlan = plan;
      show(`${modalHead2("\u0645\u062A\u0649 \u062A\u0631\u064A\u062F \u062A\u0637\u0628\u064A\u0642 \u0627\u0644\u062E\u0637\u0629\u061F")}<p>\u0627\u062E\u062A\u0631 \u0645\u0648\u0639\u062F \u0628\u062F\u0621 \u0627\u0644\u0628\u0631\u0646\u0627\u0645\u062C \u0648\u0627\u0644\u0645\u0633\u062A\u0648\u0649 \u0648\u0627\u0644\u062C\u062F\u0648\u0644 \u0627\u0644\u062C\u062F\u064A\u062F.</p><div class="v-plan-timing"><h3>\u0645\u0646 \u0647\u0630\u0627 \u0627\u0644\u0623\u0633\u0628\u0648\u0639 \xB7 ${weekStart(todayKey())}</h3><p>\u0633\u062A\u064F\u0645\u0633\u062D \u0628\u064A\u0627\u0646\u0627\u062A \u0627\u0644\u062A\u062F\u0631\u064A\u0628 \u0648\u062C\u0644\u0633\u0627\u062A \u0647\u0630\u0627 \u0627\u0644\u0623\u0633\u0628\u0648\u0639\u060C \u0628\u0645\u0627 \u0641\u064A\u0647\u0627 \u0627\u0644\u062C\u0644\u0633\u0629 \u0627\u0644\u062C\u0627\u0631\u064A\u0629 \u0648\u0627\u0644\u0645\u0646\u062C\u0632\u0629. \u062A\u0628\u0642\u0649 \u0627\u0644\u0623\u0633\u0627\u0628\u064A\u0639 \u0627\u0644\u0633\u0627\u0628\u0642\u0629 \u0645\u062D\u0641\u0648\u0638\u0629.</p>${button("\u0645\u0633\u062D \u062C\u0644\u0633\u0627\u062A \u0627\u0644\u0623\u0633\u0628\u0648\u0639 \u0648\u062A\u0637\u0628\u064A\u0642 \u0627\u0644\u062E\u0637\u0629", "apply-plan", "primary full", 'data-timing="this-week"')}<h3>\u0645\u0646 \u0627\u0644\u0623\u0633\u0628\u0648\u0639 \u0627\u0644\u0642\u0627\u062F\u0645 \xB7 ${shift(weekStart(todayKey()), 7)}</h3><p>\u062A\u0628\u0642\u0649 \u062E\u0637\u0629 \u0647\u0630\u0627 \u0627\u0644\u0623\u0633\u0628\u0648\u0639 \u0648\u062C\u0644\u0633\u0627\u062A\u0647 \u0643\u0645\u0627 \u0647\u064A\u060C \u0648\u062A\u0628\u062F\u0623 \u0627\u0644\u062C\u062F\u064A\u062F\u0629 \u064A\u0648\u0645 \u0627\u0644\u0623\u062D\u062F \u0627\u0644\u0642\u0627\u062F\u0645.</p>${button("\u062A\u0637\u0628\u064A\u0642 \u0645\u0646 \u0627\u0644\u0623\u0633\u0628\u0648\u0639 \u0627\u0644\u0642\u0627\u062F\u0645", "apply-plan", "secondary full", 'data-timing="next-week"')}${button("\u0625\u0644\u063A\u0627\u0621", "close", "text-btn")}</div>`);
    });
  });
  document.addEventListener("click", (e) => {
    ...
    if (a === "apply-plan") {
      ...
      run(async () => {
        ...
        toast2(b.dataset.timing === "this-week" ? "\u062A\u0645 \u062A\u0637\u0628\u064A\u0642 \u0627\u0644\u062E\u0637\u0629 \u0648\u0625\u0639\u0627\u062F\u0629 \u0636\u0628\u0637 \u0627\u0644\u0623\u0633\u0628\u0648\u0639" : `\u062A\u0628\u062F\u0623 \u0627\u0644\u062E\u0637\u0629 \u0627\u0644\u062C\u062F\u064A\u062F\u0629 \u064A\u0648\u0645 \u0627\u0644\u0623\u062D\u062F ${settings.revisions.at(-1).effectiveFrom}`);
      });
      return;
    }
    if (a === "detail-level" || a === "detail-session") {
      ...
      show(programComparison(b.dataset.id, level, session));
  ...
  function programComparison(p, level = settings.levels?.[p] || 1, session = "A") {
    return renderProgramDetails(compileWorkout(content, `${p}-${session.toLowerCase()}-${level}`), modalHead2);
  }
