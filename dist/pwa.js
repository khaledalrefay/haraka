  ...
  if(location.protocol !== 'file:' && 'serviceWorker' in navigator&&window.isSecureContext){
    window.addEventListener('load',()=>{
      navigator.serviceWorker.register('./sw.js',{scope:'./',updateViaCache:'none'})
        .then(registration=>{workerError=false;const update=()=>{if(registration.waiting&&navigator.serviceWorker.controller){message='تحديث جديد جاهز. أغلق جميع نوافذ حركة وافتحه لتطبيقه؛ جلستك محفوظة.';refresh();}};update();registration.addEventListener('updatefound',()=>{registration.installing?.addEventListener('statechange',update);});refresh();})
