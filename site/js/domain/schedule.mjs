...
function opportunity(settings, history, day = todayKey()) {
  ...
  if (!slot || history.some((r) => (r.scheduledDate || r.dateKey) === slot.date)) return null;
...
function updatePlan(settings, plan, { active = null, history = [], day = todayKey() } = {}) {
  const old = settings.revisions.at(-1);
  const protectedDay = Boolean(active) || history.some((r) => r.dateKey === day || r.completedDate === day);
  if (old && (old.effectiveFrom <= day || protectedDay) && old.program === plan.program && old.level === plan.level && old.schedule.every((a) => plan.schedule.some((b) => a.session === b.session && a.day === b.day))) return settings;
  return changePlan(settings, plan, day, { applyToday: !protectedDay });
}
