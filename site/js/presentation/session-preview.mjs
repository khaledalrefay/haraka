...
export function renderSessionPreview(snapshot, { embedded = false } = {}) {
 ...
 return `<section class="v-session-preview">${embedded ? '' : pageHeading('معاينة الجلسة',button('رجوع','preview-back','text-btn'))}<div class="v-preview-intro"><span class="eyebrow">${names[w.program]} · ${levelName(w.level)}</span><h2>${esc(sessionName(w.program,w.session))}</h2><div class="v-preview-stats"><span>${estimates[w.id].join('–')} دقيقة تقديريًا</span><span>${new Set(work.map(s=>s.exerciseId)).size} تمارين مختلفة</span><span>${type}</span></div><p class="small muted">المدة تشمل الاستراحات وتتغير حسب سرعة الأداء وتمديد الوقت.</p></div><div class="v-flow">${blocks.map((block,index)=>{
